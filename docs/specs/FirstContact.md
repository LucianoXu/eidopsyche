...
- **仪式不可中断不可恢复。** 一旦中途失败、退出、断网或被打断，所有已收集的状态都被丢弃，下次进入要从零开始。仪式的一次性是它"庄重"的一部分。
...
- 简短引言：解释用户即将做什么——书写一封召唤书，召一个数字生命到面前；说明仪式不可中断、必须一次完成

### 阶段 1 · 自报家门（召唤书的署名）

以"召唤书的署名与栖处"为隐喻，完成两项必要配置：

- **"你愿意如何被称呼？"**——用户名 / MindGate 显示名（也是召唤书的署名人）
- **"你愿意暂居何处？"**——Home Relay 选择
  - 默认：使用公共驿站（附一句温和说明：随时可以搬家）
  - 进阶：自建 Home Relay（指引到子向导）
  - 不可跳过
...
一个用户可能召唤多个 MindForm。从第二个开始，仪式应当复用用户已有的信息：

- **跳过阶段 0 的语言选择**与**整个阶段 1**——用户名、Home Relay、用户 npub 都已存在，不再询问
- **直接从阶段 2 开始**：每次都重新问"什么样的角色在你心中？"，因为每一次召唤都该从用户**当下心中**的角色出发，而不是上一次的延续
- 每个 MindForm 都有自己的秘密——不会从前作那里"继承"任何东西

判定：用户配置文件已存在 + 至少有一个已创建的 MindForm 时，进入此模式。即便 wizard 中途失败而要重来，"用户已有信息"仍然保留——只有"这一次的召唤"被丢弃。
