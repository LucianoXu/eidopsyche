...
func TestWrapSetupToken_RoundTrip(t *testing.T) {
	...
	// the hard way during deploy test 004 on v0.13.0. t.Fatal so we
	// don't panic on the index below if a future regression empties the
	// slice.
	if len(got.ClaudeAiOauth.Scopes) == 0 {
		t.Fatal("scopes empty; claude runtime check would reject the blob")
	}
